...
def _compute_bbox_by_cam_frustum_bounded(cfg, HW, Ks, poses, i_train, near, far):
...
def _compute_bbox_by_cam_frustum_unbounded(cfg, HW, Ks, poses, i_train, near_clip):
...
def compute_bbox_by_cam_frustum(args, cfg, HW, Ks, poses, i_train, near, far, **kwargs):
    print("compute_bbox_by_cam_frustum: start")
    if cfg.data.unbounded_inward:
        xyz_min, xyz_max = _compute_bbox_by_cam_frustum_unbounded(
            cfg, HW, Ks, poses, i_train, kwargs.get("near_clip", None)
        )

    else:
        xyz_min, xyz_max = _compute_bbox_by_cam_frustum_bounded(
            cfg, HW, Ks, poses, i_train, near, far
        )
    print("compute_bbox_by_cam_frustum: xyz_min", xyz_min)
    print("compute_bbox_by_cam_frustum: xyz_max", xyz_max)
    print("compute_bbox_by_cam_frustum: finish")
    return xyz_min, xyz_max
...
def train(args, cfg, data_dict):
    ...
    xyz_min_coarse, xyz_max_coarse = compute_bbox_by_cam_frustum(
...
if __name__ == "__main__":
    ...
    if args.export_bbox_and_cams_only:
        ...
        xyz_min, xyz_max = compute_bbox_by_cam_frustum(args=args, cfg=cfg, **data_dict)
        poses, HW, Ks, i_train = (
            data_dict["poses"],
            data_dict["HW"],
            data_dict["Ks"],
