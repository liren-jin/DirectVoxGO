...
class DenseGrid(nn.Module):
    ...
    def forward(self, xyz):
        ...
        """
        shape = xyz.shape[:-1]  # get point mumber M
        xyz = xyz.reshape(1, 1, 1, -1, 3)  # (1, 1, 1, M, 3) for trilinear interpolation
        ...
        out = out.reshape(self.channels, -1).T.reshape(*shape, self.channels)  # (M, C)
        if self.channels == 1:
            out = out.squeeze(-1)
        return out
